...
while IFS= read -r command; do
    if [[ -n "$command" ]]; then
        tmux send-keys -t fedora-vm-base "$command" C-m
        sleep 1
    fi
...
useradd -m -g users -G wheel,storage,power -s /bin/bash fedora
...
tmux send-keys -t fedora-vm-base "$INSTALLATION_SCRIPT" C-m 
...
tmux send-keys -t fedora-vm-base "$EXECUTE_INSTALL_SCRIPT" C-m
